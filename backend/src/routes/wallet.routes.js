...
router.get('/', protect, async (req, res) => {
    try {
        ...
        res.status(200).json({ success: true, data: { balance: user?.walletBalance || 0, transactions } });
    } catch (e) {
        res.status(500).json({ success: false, message: e.message });
    }
});
...
router.post('/topup', protect, async (req, res) => {
  try {
    ...
    await WalletTransaction.create({ user: req.user._id, type: 'TOPUP', amount, description: 'Recarga manual' });
    res.json({ success: true, data: { newBalance: user.walletBalance } });
  } catch (e) {
    res.status(500).json({ success: false, message: e.message });
  }
});
